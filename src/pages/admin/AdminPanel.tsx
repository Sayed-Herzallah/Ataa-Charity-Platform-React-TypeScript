    ...
    <aside className={`ap-sidebar${collapsed ? ' collapsed' : ''}`}>
      ...
      {!collapsed && (
        <div className="ap-live-pill">
          ...
          <span className="ap-live-time" id="ap-clock">--:--</span>
        </div>
      )}
      ...
      <div className="ap-sidebar-footer">
        {/* PATCH 3: sidebar user block clickable → settings */}
        {!collapsed && (
          <div
  ...
  return (
    <div className="ap-card-grid">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="ap-entity-card" style={{ pointerEvents: 'none' }}>
          <div style={{ display: 'flex', gap: 12, marginBottom: 14 }}>
            <div className="ap-skel" style={{ width: 42, height: 42, borderRadius: 11, flexShrink: 0 }} />
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            <div className="ap-skel" style={{ height: 22, width: 74, borderRadius: 20 }} />
          </div>
        </div>
      ))}
    </div>
    ...
    <div className="ap-chart-tooltip">
      ...
      {payload.map((p, i) => (
        <div key={i} className="ap-chart-tooltip-row">
          <span>{p.name}</span>
          <span style={{ fontWeight: 700 }}>{p.value.toLocaleString('ar-EG')}</span>
        </div>
      ))}
    </div>
  );
...
export default function AdminPanel() {
  ...
  // ── Chart data ────────────────────────────────────────────────────────────
  const trendData = [
    { name: 'يناير', users: Math.round((usersTotal || 10) * 0.45), charities: Math.round((charitiesTotal || 5) * 0.55) },
    { name: 'فبراير', users: Math.round((usersTotal || 10) * 0.55), charities: Math.round((charitiesTotal || 5) * 0.65) },
    { name: 'مارس',  users: Math.round((usersTotal || 10) * 0.65), charities: Math.round((charitiesTotal || 5) * 0.72) },
    { name: 'أبريل', users: Math.round((usersTotal || 10) * 0.72), charities: Math.round((charitiesTotal || 5) * 0.80) },
    { name: 'مايو',  users: Math.round((usersTotal || 10) * 0.82), charities: Math.round((charitiesTotal || 5) * 0.88) },
    { name: 'يونيو', users: usersTotal || 10,                       charities: charitiesTotal || 5 },
  ];
  ...
  return (
    <div className="ap-layout" dir="rtl">
      ...
      <main className={`ap-main${tab === 'ai-chat' ? ' ap-main--ai' : ''}`}>
        ...
        {/* Page header — hidden when AI chat is active */}
        {tab !== 'ai-chat' && (
          <header className="ap-page-header">
            ...
            <div className="ap-page-header-right">
              ...
              {/* PATCH 4: header avatar → settings */}
              <div
            ...
            <>
              ...
              {tab === 'overview' && (
                <div className="ap-tab-pane">
                  ...
                  <div className="ap-chart-card">
                    <div className="ap-chart-header">
                      <span className="ap-chart-title"><i className="ti ti-chart-bar" style={{ color: '#3b82f6' }} />مقارنة شهرية</span>
                    </div>
                    <ResponsiveContainer width="100%" height={190}>
                      <BarChart data={trendData} margin={{ top: 8, right: 4, left: -20, bottom: 0 }} barGap={4}>
                        <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" vertical={false} />
                        <XAxis dataKey="name" tick={{ fill: 'var(--t3)', fontSize: 11 }} axisLine={false} tickLine={false} />
                        <YAxis tick={{ fill: 'var(--t3)', fontSize: 11 }} axisLine={false} tickLine={false} allowDecimals={false} />
                        <Tooltip content={<ChartTooltip />} />
                        <Bar dataKey="users" name="المستخدمون" fill={TEAL2} radius={[4,4,0,0]} maxBarSize={32} />
                        <Bar dataKey="charities" name="الجمعيات" fill="#3b82f6" radius={[4,4,0,0]} maxBarSize={32} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
                      ...
                      <table className="ap-table">
                        ...
                        <tbody>
                          {filteredUsers.map(u => {
                            ...
                            return (
                              <tr key={u._id} className="ap-table-row-clickable">
                                <td>
                      ...
                      {filteredCharities.map(c => (
                        <div key={c._id} className="ap-entity-card" onClick={() => setCharityDetailModal(c)}>
                          ...
                          <div className="ap-entity-actions" onClick={e => e.stopPropagation()}>
                            {/* PATCH 6: eye button first */}
                            <button className="ap-card-eye-btn" onClick={() => setCharityDetailModal(c)} title="عرض التفاصيل">
                      ...
                      {filteredReports.map((r, i) => {
                        ...
                        return (
                          <div key={r._id} className="ap-report-card" onClick={() => setReportModal(r)}>
                            ...
                            {/* PATCH 7: eye button on report card */}
                            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 4 }}>
                              <button className="ap-card-eye-btn" onClick={e => { e.stopPropagation(); setReportModal(r); }} title="عرض التفاصيل">
                ...
                <div className="ap-tab-pane">
                  ...
                  <div className="ap-settings-grid">
                    ...
                    <div className="ap-settings-card">
                      ...
                      {[
                        { label: 'الوضع الليلي',   sub: 'تفعيل المظهر الداكن',   default: true  },
              ...
              {/* ══ AI CHAT ══════════════════════════════════════════════════ */}
              {/* PATCH 8: full viewport, no header, no scroll */}
              {tab === 'ai-chat' && (
