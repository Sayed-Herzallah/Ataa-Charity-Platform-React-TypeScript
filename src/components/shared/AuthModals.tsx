...
export default function AdminPanel() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [uRes, cRes, uAll, cAll] = await Promise.allSettled([
        fetchPage<User>('/users', 1, 10),
        fetchPage<Charity>('/charity/charities', 1, 10),
        fetchPage<User>('/users', 1, 1000),
        fetchPage<Charity>('/charity/charities', 1, 1000),
      ]);
      ...
      if (uAll.status === 'fulfilled') {
        setAllUsersForTrend(uAll.value.data ?? []);
      }
      if (cAll.status === 'fulfilled') {
        setAllCharitiesForTrend(cAll.value.data ?? []);
      }

      // Fetch reports using apiFetch (the shared API client)
      ...
      try {
        ...
        reportsTotalCount = pageRes.total ?? reportsList.length;
      } catch (err) {
        // fetchPage failed — try a raw apiFetch as fallback
        try {
          const repRes = await apiFetch<any>('/report/allReports');
          ...
          console.error('Failed to fetch reports:', innerErr);
          // Reports unavailable — leave as empty array, do not block the rest of the dashboard
        }
      }
  ...
  const trendData = useMemo(() => {
    ...
    allCharitiesForTrend.forEach(c => {
      if (c.createdAt) {
  ...
  const dynamicTrend = useMemo(() => {
    ...
    const currentMonthCount = 
      allUsersForTrend.filter(u => u.createdAt && new Date(u.createdAt).getMonth() === currentMonth && new Date(u.createdAt).getFullYear() === currentYear).length +
      allCharitiesForTrend.filter(c => c.createdAt && new Date(c.createdAt).getMonth() === currentMonth && new Date(c.createdAt).getFullYear() === currentYear).length;

    const lastMonth = currentMonth === 0 ? 11 : currentMonth - 1;
    const lastMonthYear = currentMonth === 0 ? currentYear - 1 : currentYear;

    const lastMonthCount = 
      allUsersForTrend.filter(u => u.createdAt && new Date(u.createdAt).getMonth() === lastMonth && new Date(u.createdAt).getFullYear() === lastMonthYear).length +
      allCharitiesForTrend.filter(c => c.createdAt && new Date(c.createdAt).getMonth() === lastMonth && new Date(c.createdAt).getFullYear() === lastMonthYear).length;
  ...
  const charityTrend = useMemo(() => {
    ...
    const curr = allCharitiesForTrend.filter(c => c.createdAt && new Date(c.createdAt).getMonth() === currentMonth && new Date(c.createdAt).getFullYear() === currentYear).length;
    const prev = allCharitiesForTrend.filter(c => c.createdAt && new Date(c.createdAt).getMonth() === lastMonth && new Date(c.createdAt).getFullYear() === lastMonthYear).length;
        ...
        <div ref={contentRef} className={`ap-content${tab === 'ai-chat' ? ' ap-content--ai' : ''}`}>
          ...
          {loading ? <PageSkeleton /> : (
            <>
              {/* ══ OVERVIEW ══════════════════════════════════════════════════ */}
              {tab === 'overview' && (
                <div className="ap-tab-pane" style={{ height: 'fit-content', gap: 10 }}>
                  <div className="ap-kpi-grid">
                  ...
                  <div className="ap-dashboard-grid">
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, minWidth: 0 }}>
                      <div className="ap-pro-chart-card ap-chart-card--wide" style={{ width: '100%' }}>
                        {/* Header */}
                        <div className="ap-pro-chart-header">
                          <div className="ap-pro-chart-meta">
                            <div className="ap-pro-chart-value">
                              {((usersTotal || allUsersForTrend.length) + (charitiesTotal || allCharitiesForTrend.length)).toLocaleString('en-US')}
                            </div>
                        ...
                        {/* Mini stats — match chart (real totals) */}
                        <div className="ap-chart-stats-row">
                          <div className="ap-chart-stat-mini">
                            <div className="ap-chart-stat-mini-val" style={{color:TEAL2}}>{(usersTotal || allUsersForTrend.length).toLocaleString('en-US')}</div>
                          ...
                          <div className="ap-chart-stat-mini">
                            <div className="ap-chart-stat-mini-val" style={{color:'#3b82f6'}}>{(charitiesTotal || allCharitiesForTrend.length).toLocaleString('en-US')}</div>
                          ...
                          <div className="ap-chart-stat-mini">
                            <div className="ap-chart-stat-mini-val" style={{color:AMBER}}>{(reportsTotal || allReportsForTrend.length).toLocaleString('en-US')}</div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, minWidth: 0 }}>
                      ...
                      {reports.length > 0 && (
                        <div className="ap-chart-card">
                          ...
                          <div style={{ marginTop: 10, display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--t4)', padding: '0 2px' }}>
                            <span>إجمالي التقارير: <strong style={{ color: 'var(--t2)' }}>{reportsTotal || reports.length}</strong></span>
                          </div>
                        </div>
                      )}
                    </div>
                      ...
                      <div className="ap-filter-tabs">
                        {(['all', 'pending', 'approved', 'rejected'] as const).map(f => (
                          <button key={f} className={`ap-filter-tab${charitiesFilter === f ? ' active' : ''}`} onClick={() => handleCharitiesFilterChange(f)}>
                            {f === 'all' ? `الكل (${charitiesTotal || allCharitiesForTrend.length})` : f === 'pending' ? 'قيد المراجعة' : f === 'approved' ? 'مقبول' : 'مرفوض'}
                            {f === 'pending' && pendingCount > 0 && (
                              <span className="ap-filter-badge" style={{ marginRight: 4 }}>{pendingCount}</span>
                            )}
                          </button>
                        ))}
                        {loadingAllCharities && (
              ...
              {tab === 'settings' && (
                <div className="ap-tab-pane">
                  ...
                  <div className="ap-settings-card" style={{ background: 'rgba(244,63,94,0.06)', border: '1px solid rgba(244,63,94,0.2)', borderRadius: 'var(--radius)', padding: '20px 22px', marginTop: 24 }}>
